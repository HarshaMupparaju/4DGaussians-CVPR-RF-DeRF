...
class Neural3D_NDC_Dataset(Dataset):
    def __init__(
        ...
        if(dataset_name == "dynerf"):
            ...
            self.downsample = 2048 / self.img_wh[0]
    ...
    def load_meta(self):
        ...
        # if(self.dataset_name == "dynerf"):
        poses_arr = np.load(os.path.join(self.root_dir, "poses_bounds.npy"))
        poses = poses_arr[:, :-2].reshape([-1, 3, 5])  # (N_cams, 3, 5)
        self.near_fars = poses_arr[:, -2:]
        # elif(self.dataset_name == "interdigital"):
        #     intrinsics = np.loadtxt(os.path.join(self.root_dir, "CameraIntrinsics_undistorted.csv"), delimiter=',')
        #     extrinsics = np.loadtxt(os.path.join(self.root_dir, "CameraExtrinsics_undistorted.csv"), delimiter=',')
        #     bounds = np.loadtxt(os.path.join(self.root_dir, "DepthBounds.csv"), delimiter=',')
